...
bool SpawnObjectService::Init(std::shared_ptr<DdsDomainParticipantImpl> domain_participant) {
  _impl->SetSyncServiceCallback(std::bind(&SpawnObjectService::SpawnObject, this, std::placeholders::_1));
  ...
  do {
    ...
    if (blueprints->empty()) {
      ...
      if (result.HasError()) {
        ...
        log_debug("ROS2:SpawnObjectService spawn succeeded: ", int32_t(result.Get().id));
        return response;
      }
    }
  }while(retry_count > 0);
  
  log_error("ROS2:SpawnObjectService failed to retrieve random spawn point after retries");
  response.error_string("SpawnObjectService: failed to retrieve random spawn point after retries");
  response.id(-1);
  return response;
}
