...
public:
  ...
  virtual ~DdsPublisherImpl() {
    carla::log_debug("DdsPublisherImpl[", _topic != nullptr ? _topic->get_name() : "nulltopic", "]::Destructor()");
    if (_datawriter) {
...
private:
  ...
  void on_publication_matched(eprosima::fastdds::dds::DataWriter*,
    ...
    carla::log_debug("DdsPublisherImpl[", _topic->get_name(), "]::on_publication_matched(): ", _matched);
  }
